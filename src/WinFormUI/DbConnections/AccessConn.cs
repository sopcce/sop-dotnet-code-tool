...
namespace CodeTool.DbConnections
{
    public partial class AccessConn : UserControl, IDbConn
    {
        ...
        private string GetTable()
        {
            ...
            connStr.Append(SqlConfig.GetAccessProvider);

            if (!chkNullPassword.Checked)
            {
                connStr.Append(string.Format("Password={0};", txtPasswrod.Text.Trim()));
            }
            connStr.Append(string.Format(SqlConfig.GetDataSource, txtPath.Text.Trim()));
            connStr.Append(SqlConfig.GetPersistSecurityInfo);
